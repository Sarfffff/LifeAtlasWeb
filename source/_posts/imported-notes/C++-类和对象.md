...
属性 - C/C++ - 输出文件 - 汇编程序输出 原本这里应该是无列表 选择仅有程序集的列表/FA 再build 那么就可以得到一个 .asm 文件（[另一种查看汇编的方案](https://chocomintopia.github.io/1-Cherno-C++.html#mypoint_2)） 这个就是汇编语言 不再是机器代码了 而是汇编语言 这是CPU将要执行的实际指令 可以看到函数名字前面有一堆看似乱码的东西 这是函数的签名 可以唯一地定义你的函数 如果我们有多个obj 函数也被定义在多个obj中 链接的工作就是把所有的函数链接在一起

我们也可以从汇编文件中看到 变量设置得很多的话 实际上是影响效率 比如你可以直接返回 a+b 而不是再设置一个变量 c=a+b 再返回 c 这样的话会多出来很多针对于变量的 mov 指令

debug模式下也不会给你做优化 在属性 - C/C++ - 优化 - 优化 原本应该是 已禁用/Od 选择 最大优化/O2 同时上方配置那里从 所有配置 改成 Debug 然后再build 它就会报错 告诉你O2和RTC不兼容 现在我们要继续去 属性 - C/C++ - 代码生成 - 基本运行时检查 原本应该是 两者(/RTC1，等同于 /RTCsu) (/RTC1) 在这里选择 默认值 就不会再执行运行时检查 再看 .asm 汇编文件 就会发现 文件变得小多了 比如减少了一些针对变量的 mov 指令
...
如果在函数前面加一个 [static](https://chocomintopia.github.io/1-Cherno-C++.html#mypoint_7) 就说明这个函数只在当前cpp文件里会被使用 其它cpp文件里都不会用到 那么它就不用参与链接 其他cpp文件就不使用
...
**现在就用pragma once**(主要是防止嵌套包含的问题出现)
...
|  名称  |                             值                              |     类型     |
| :----: | :---------------------------------------------------------: | :----------: |
|   a    | -858993460 （可以在这里右键 让它16进制显示 就是0xcccccccc） |     int      |
| string |              0xcccccccc <读取字符串字符时出错               | const char * |
...
[利用汇编debug](https://chocomintopia.github.io/1-Cherno-C++.html#mypoint_2)
...
1.写在类外，结构体外，函数外的static int 变量 代表只在这个翻译单元可见，其他的cpp文件看不见

2.类中的 `static`:代表x，y属于 **Entity 这个类**，不属于某个 `Entity` 对象无论创建多少个对象，这两个变量都只有一份。同样的，静态成员函数也是属于类，而非某个具体的实例对象，同时静态成员函数不能访问非静态成员变量（因为静态函数共享的，而每个非静态成员变量是非共享的，他不知道你想访问那个）

**普通成员变量是“每个对象各有一份” 静态成员变量是“整个类共享一份”**

```C++
#include<iostream>
struct Entity {  //这个只是声明，还没有定义，需要在类外进行定义
    static int x;
    static int y;
};
int Entity::x = 0; //等同于e1.x = 2;
int Entity::y = 0;
int main() {
    Entity e1;
    e1.x = 2;
...
class Log{
private:
    Log() = delete; // 构造函数被删除了或者private，则这个类对象不能够被实例化
public:
	static void Write()
    {
...
**如果子类的对象和父类的对象同名，默认访问子类的对象，如果需要访问父类，需要加上作用域 父类：：。如果出现子类和父类的函数名相同，默认是访问子类的函数，并且父类的同名方法全部被隐藏，如果需要访问也是需要加上作用域	**		

​										

**使用虚继承可以解决菱形继承的问题。当派生类（如Derived1和Derived2）以`virtual`关键字继承自同一个基类（Base）时，编译器会为这两个派生类创建虚基类指针（vptr）或类似的机制。这些指针指向同一个虚基类表（vtable），表中包含虚基类子对象的位置信息。最终派生类（如Final）中，通过这两个虚基类指针找到的是同一个Base子对象，从而保证了数据的唯一性，消除了歧义。**



**虚继承通过引入虚基类指针（vbptr）和虚基类表（vbtable）来解决菱形继承问题。在虚继承体系中，每个包含虚基类的对象都包含一个指向虚基类表的指针，该表中记录了虚基类子对象相对于完整对象的偏移量。在菱形继承结构中，多个中间基类（如Derived1和Derived2）的虚基类表都指向同一个Base子对象的偏移信息，从而保证最终派生类（Final）中只有一个Base子对象，避免了数据冗余和访问歧义。**
...
class Final : public Derived1, public Derived2 {
    // 问题：从两个路径继承了Base，有两个data副本
};

Final obj;
// obj.data = 200;           // 错误：不明确
// cout << obj.data;         // 错误：不明确
```

采用作用域的方式，明确每个类的元素

```C++
obj.Derived1::data = 200;     // 明确指定通过Derived1路径
obj.Derived2::data = 300;     // 明确指定通过Derived2路径
cout << obj.Derived1::data;   // 输出200
cout << obj.Derived2::data;   // 输出300
```
...
虚析构：当基类指针指向派生类对象时，如果基类的析构函数不是虚函数，删除基类指针只会调用基类的析构函数，导致派生类的部分资源泄漏。也就是父类指针在析构的时候，不会调用子类的析构函数，如果子类有堆区的属性，那么则不会释放，造成内存泄漏。前提是基类指针指向派生类对象（用基类来实例化子类）
...
int main() {
    ...
    delete ptr;  // 正确：会调用 Derived::~Derived() 和 Base::~Base()
...
public:
    ...
    ~BadDerived() { delete[] data; }  // 永远不会被调用！
};

// 使用：
BadBase* ptr = new BadDerived();
delete ptr;  // 内存泄漏！只调用BadBase::~BadBase()
```

纯虚析构：纯虚析构函数可以使一个类成为抽象类，但必须**提供定义**（与纯虚成员函数不同）。
...
函数末尾的`const`**只能用于类的成员函数**，普通函数加会直接编译错误。它的核心作用是：**限定这个成员函数不能修改所属对象的任何成员变量**
...
**参数列表里的`const`是用来**限定函数的形参**，表示这个参数在函数内部**不允许被修改**，本质是给参数加上 “只读” 属性。**
...
// const修饰普通参数（值传递）：防止函数内修改形参副本
void printValue(const int num) {
    // num = 100; // 编译错误！const参数不能被修改
    cout << "数值：" << num << endl;
}

// const修饰指针参数：防止通过指针修改指向的内容
void printString(const char* str) {
    // str[0] = 'A'; // 编译错误！不能修改指针指向的内容
    cout << "字符串：" << str << endl;
}

// const修饰引用参数（最常用）：避免拷贝+防止修改原数据
void printName(const string& name) {
    // name = "Li Ming"; // 编译错误！const引用不能修改
    cout << "姓名：" << name << endl;
}
...
在[Visual Studio设置](https://chocomintopia.github.io/1-Cherno-C++.html#Visual Studio设置) 我们似乎做过类似的工作
...
指的是解决方案所在目录(ProjectDir) 指的是项目文件.vcxproj所在目录
...
1. 第一种方法 我可以设置项目的包含路径 `$(SolutionDir)dependencies\GLFW\include`
...
按照[Visual Studio设置](https://chocomintopia.github.io/1-Cherno-C++.html#visual-studio设置)修改输出目录和中间目录 以及创建src文件夹
...
在堆上分配内存请查看[new](https://chocomintopia.github.io/1-Cherno-C++.html#new) 是一系列的事情 而在栈上分配内存就只类似于一条指令 所以栈分配会更快 这是它们最主要的差别 查看反汇编就可以看到差异
...
现在迭代器不太常用 推荐用[for range](https://chocomintopia.github.io/1-Cherno-C++.html#mypoint_14)
...
其实PrintValue这么一点信息 根本不用写成函数 特别是我们只想在ForEach内部使用 这就可以使用[lambda](https://chocomintopia.github.io/1-Cherno-C++.html#mypoint_15) 其实就是一个匿名函数 只是不像普通函数这样声明
...
只要有一个函数指针 就可以在C++中使用lambda 这是不需要通过函数定义就可以定义一个函数的方法
...
我们现在已经知道 base实际上是Derived类的一个实例 但我们假装不知道
...
[dynamic_cast](https://chocomintopia.github.io/1-Cherno-C++.html#mypoint_16)更像是一个函数 它不是编译时进行的类型转换 而是在运行时计算 所以它会有运行成本
...
[shared_ptr](https://chocomintopia.github.io/1-Cherno-C++.html#mypoint_17)
...
能让我们更好地处理[多返回值 ](https://chocomintopia.github.io/1-Cherno-C++.html#多返回值)可以用tuple pair 也可以用结构体
...
回到当时我们那个[Shader的例子](https://chocomintopia.github.io/1-Cherno-C++.html#mypoint_18)
...
std::variant和[union](https://chocomintopia.github.io/1-Cherno-C++.html#union)不是一样的
...
[在堆上进行内存分配](https://chocomintopia.github.io/1-Cherno-C++.html#new)不一定是坏事 但是要尽量避免
...
复用了之前[基准测试](https://chocomintopia.github.io/1-Cherno-C++.html#基准测试)的Timer类
...
运行RunBenchmarks函数时 因为预处理器__FUNCTION__取的是函数的实际名称 也就是PrintFunction

我们想要函数签名 也就是__FUNCSIG__
...
[singleton的static](https://chocomintopia.github.io/1-Cherno-C++.html#mypoint_19)
...
[之前](https://chocomintopia.github.io/1-Cherno-C++.html#union)我们没有提到联合体的构造函数 其实联合体是可以有构造函数的 这个`_Bxty`类型联合体的构造函数`_Bxty() noexcept : _Buf() {}` 只是创建了一个`_Buf[_BUF_SIZE]`数组 实际上也等同于创建了一个指针`_Ptr` 但**联合体不能同时激活多个成员** 于是在构造时选择了初始化`_Buf` 那么就是默认为小字符串 而在使用`_Ptr`(堆分配)前需要先通过placement new激活 我们目前还不知道`_Buf`数组的`_BUF_SIZE` 所以在创建之前需要设置好`_BUF_SIZE` 而且我们也不知道联合体里的`_Ptr`在哪里激活
...
当你写`delete _Ptr` 编译器会根据对象类型和上下文 选择合适的operator delete重载 从C++17开始 如果编译器知道对象的大小 (比如有类型信息) 它就会优先调用带size_t参数的`operator delete(void*, size_t)` 而不是`operator delete(void*)` 我们在使用` std::make_unique<Object>()`分配对象时 编译器已经能确定Object的大小 所以在delete时会选择带有size的重载
...
**C++中浅拷贝与深拷贝的核心区别在于内存管理：浅拷贝只复制指针值，导致多个对象共享同一块堆内存，这会引发致命错误——当一个对象析构释放内存后，其他对象将持有悬垂指针，后续访问或析构会导致程序崩溃（双重释放）。深拷贝则为每个对象分配独立内存，复制实际数据，确保内存安全。**

**在实际开发中，应优先使用标准库容器（如`std::string`、`std::vector`）或智能指针（`std::unique_ptr`），它们已内置正确的拷贝语义。若必须管理原始指针资源，需遵循规则三/五法则，手动实现深拷贝构造函数、拷贝赋值运算符和析构函数，从根源上避免内存共享引发的双重释放问题。**

# 对象特性const修饰成员函数

## mutable

类型修饰符，用于修改类的非静态成员，使其：

- 即使在 `const`成员函数中也可以被修改
- 即使对象是 `const`的也可以被修改

```C++
class MyClass {
private:
    mutable int mutableData;    // 可变成员
    int normalData;             // 普通成员
    
public:
    void constFunction() const {
        // mutableData = 10;    // ✓ 允许：mutable成员可以在const函数中修改
        // normalData = 20;     // ✗ 错误：普通成员不能在const函数中修改
    }
};
```
...
- 不对称性：友元关系是**单向的**，不是双向的。就像"你是我的朋友"不等于"我是你的朋友"。
- 不可传递性：友元关系**不会传递**。如果A是B的朋友，B是C的朋友，不代表A是C的朋友。
- 不可继承性：基类的友元**不是派生类的友元**。

#### 友元全局函数

当一个外部函数需要频繁访问类的私有数据时，可以声明为友元。

**实际场景**：转账函数需要同时操作两个账户的余额，如果通过公开接口，需要多次验证和额外开销。友元让转账操作更高效。
...
bool transfer(BankAccount& from, BankAccount& to, double amount) {
    if (from.balance >= amount) {  // 直接访问私有balance
        from.balance -= amount;     // 直接修改
        to.balance += amount;
        return true;
    }
    return false;
...
当一个类需要深度访问另一个类的内部时，可以声明为友元类。

```C++
class SecuritySystem;  // 前向声明
...
private:
    ...
    // 只有SecuritySystem能访问
    friend class SecuritySystem;  // 声明友元类
};

class SecuritySystem {
public:
    // 可以访问Safe的所有私有成员
    bool unlockSafe(Safe& safe, const string& code) {
        if (safe.secretCode == code) {  // 直接访问私有成员
            safe.locked = false;
            return true;
        }
        return false;
...
class Logger {
public:
    void logConnection(const Database& db);  // 只声明
};
...
private:
    ...
    // 只允许Logger的logConnection函数访问
    friend void Logger::logConnection(const Database& db);
};

// Logger成员函数的实现
void Logger::logConnection(const Database& db) {
    // 可以访问私有成员
    cout << "连接数: " << db.connectionCount << endl;
    cout << "最后错误: " << db.lastError << endl;
}
```

# 文件操作

C++ 通过 `<fstream>`头文件提供文件操作功能，主要包含三个类：`ifstream`、`ofstream`和 `fstream`。

## 文本文件

| 类         | 用途       | 继承自     | 主要功能       |
| ---------- | ---------- | ---------- | -------------- |
| `ifstream` | 输入文件流 | `istream`  | 从文件读取数据 |
| `ofstream` | 输出文件流 | `ostream`  | 向文件写入数据 |
| `fstream`  | 文件流     | `iostream` | 同时读写文件   |
...
int main() {
    ...
    // 或 ofstream fout("test.txt", ios::out);
    
    if (!fout.is_open()) {  // 检查是否成功打开
        cerr << "文件打开失败！" << endl;
        return 1;
    }
...
**文件打开模式**：必须使用 `ios::binary`标志，否则在Windows平台会进行换行符转换
...
**模板（Template）\**是 C++ 提供的一种 \*\*编译期泛型机制\*\*，允许将\**类型、常量或模板本身**作为参数，从而实现**代码复用 + 零运行时开销**。
...
- 普通函数是在编译前进行确定的，模板函数是在编译器进行推导的（调用时实例化）
- 普通函数只有一份函数实现，模板按需实例化，每种T生成一份函数
...
fun(1,2.5);//不能进行隐式类型转换
```

#### 模板特化和偏特化

**简单来说，**模板特化 就是为模板参数提供一个 “量身定制” 的实现版本，而**偏特化**（也叫部分特化）是特化的一种特殊形式，只针对模板参数的一部分进行定制，其余参数仍保持通用。**但是C++不支持函数模板的偏特化，支持类模板偏特化**

**模板特化**：模板的**全特化**（Full Specialization）是指为模板的**所有模板参数**都指定具体的类型 / 值，为这个特定组合提供专属的实现逻辑，它会完全覆盖通用模板的行为。

假设你写了一个通用模板函数`print<T>`，就像一个 “通用包装盒”，能装任意类型的东西：

- 正常使用时（模板实例化）：你说 “给我一个装 int 的盒子”，编译器就用通用模板造一个 int 版的盒子；
- 模板特化：你提前为 int 类型定制了一个 “带标签的专属盒子”，当你再要装 int 的盒子时，编译器会直接用这个定制版，而非通用版。
...
// 2. 为int类型做特化（提前写好专属实现）
template <>
void print<int>(int val) {
    cout << "int特化版: " << val << " (定制输出)" << endl;
}

int main() {
    // 3. 使用时：编译器自动匹配，无需手动指定（也可以指定）
    print(10);        // 自动匹配int特化版（不是临时指定，而是用提前写好的特化代码）
    print(3.14);      // 用通用模板实例化double版本（无特化）
    print<string>("abc"); // 用通用模板实例化string版本（无特化）
    
    // 即使手动指定类型，本质还是匹配提前写好的特化/通用版
    print<int>(20);   // 依然用int特化版
    return 0;
}
```

**类模板偏特化**：偏特化仅针对**部分模板参数**定制实现，其余参数仍为通用型，**仅适用于类模板**（函数模板不支持偏特化）。常见场景有两种：

- 对参数的 “数量” 偏特化（比如从多个参数特化其中一个）；
- 对参数的 “类型特征” 偏特化（比如特化指针 / 引用类型）。
...
public:
    void process(T a, U b) {
        cout << "通用版：处理 " << typeid(T).name() << " 和 " << typeid(U).name() << endl;
        cout << "结果：" << a << ", " << b << endl;
    }
};

// 偏特化1：固定第二个参数为int，第一个参数仍通用
template <typename T>
class DataProcessor<T, int> {
public:
    void process(T a, int b) {
        cout << "偏特化版（U=int）：处理 " << typeid(T).name() << " 和 int" << endl;
        cout << "结果：" << a << ", " << b << "（int类型已固定）" << endl;
    }
};

// 偏特化2：固定第一个参数为string，第二个参数仍通用
template <typename U>
class DataProcessor<string, U> {
public:
    void process(string a, U b) {
        cout << "偏特化版（T=string）：处理 string 和 " << typeid(U).name() << endl;
        cout << "结果：\"" << a << "\", " << b << "（string类型已固定）" << endl;
    }
};

int main() {
    // 1. 通用版：两个参数都不是固定值
    DataProcessor<double, double> dp1;
    dp1.process(3.14, 6.28);

    // 2. 偏特化版1：第二个参数是int
    DataProcessor<string, int> dp2;
    dp2.process("测试", 100);

    // 3. 偏特化版2：第一个参数是string
    DataProcessor<string, double> dp3;
    dp3.process("Hello", 9.9);
...
**类模板的继承**：普通类继承模板类,继承时需要指定数据类型才能继承
...
类外定义：两个 `template <typename T>` 都不能省

```C++
template <typename T>//构造函数的类外实现
...
**类模板的分文件编写**：类模板中成员函数的创建时机是在调用阶段，导致分文件编写时链接不到

类模板不能同普通类一样.h + .cpp;目标必须在实例化可见
...
友元函数模板：所有 `Box<U>` 均可访问私有成员

```C++
template <typename T>
class Box {
    template <typename U>
    friend void show(Box<U> b);
private:
    T value;
};
```
...
![image-20260208154107544](/notes-assets/C%2B%2B/assets/image-20260208154107544.png)
...
public:
	...
	MyArray& operator=(MyArray& op) {
		if (this == &op)//如果传进来的等于自身
			return *this;
...
├── 适配器 Adapters
└── 分配器 Allocators
容器存数据，算法处理数据，迭代器连接两者，其余模块是“增强件”                 
```
...
| 容器                            | 本质                         |
| ------------------------------- | ---------------------------- |
| `vector`                        | 动态数组（你刚刚手写的那个） |
| `deque`                         | 双端队列                     |
| `list`                          | 双向链表                     |
| `set / map`                     | 红黑树                       |
| `unordered_set / unordered_map` | 哈希表                       |

📌 **刚实现的 `MyArray<T>` ≈ `std::vector<T>`（简化版）**

##### vector

##### deque：

双向开口的连续线性空间，可以在两端进行高效的元素插入和删除操作

核心结构：中控器 + 数据块 + （迭代器）。每个数据块都被中控器中指针指向，每个指针都指向一个数据块

```C++
// 简化概念结构
class deque {
    T** map;           // 中控器（指针数组），每个指针指向一个数据块
...
- 每个数据块是固定大小的连续内存（通常512字节）
- 存储实际的数据元素
- 数据块大小 = `sizeof(T) * buffer_size`

当需要新空间时，`deque`不是像vector那样重新分配整个数组，而是：
...
struct DequeIterator {
    ...
    // 自减操作
    DequeIterator& operator--() {
        if (cur == first) {      // 到达缓冲区起始
            set_node(node - 1);  // 跳转到上一个缓冲区
            cur = last;          // 从末尾开始
        }
        --cur;
        return *this;
...
// 3. 指定大小和初始值
deque<T> dq(size_t n, const T& value);

// 4. 范围构造函数
template<class InputIterator>
deque<T> dq(InputIterator first, InputIterator last);

// 5. 拷贝构造函数
deque<T> dq(const deque<T>& other);

// 6. 移动构造函数
deque<T> dq(deque<T>&& other);

// 7. 初始化列表构造函数
deque<T> dq(initializer_list<T> il);
```
...
iterator insert(const_iterator pos, size_t n, const T& value);//dq.insert(dq.begin(),2,100) //在pos位置插入n个元素
...
| **修改器**    | `push_front()`、`push_back()`、`pop_front()`、`pop_back()`、`insert()`、`erase()`、`clear()` |
| **迭代器**    | `begin()`、`end()`、`rbegin()`、`rend()`                     |
| **赋值**      | `operator=`、`assign()`                                      |
| **比较**      | `operator==`、`operator<`等                                  |
| **其他**      | `swap()`、`get_allocator()`                                  |

##### List

list是C++标准库中的双向链表容器，与vector和deque不同，它不是连续存储的，而是由一系列节点通过指针连接而成。每个节点都包含数据和指向前后节点的指针。
...
class list {
private:
    ListNode<T>* head;     // 头节点（哨兵节点）
    ListNode<T>* tail;     // 尾节点（哨兵节点）
    size_t size;          // 元素数量
};
```

**现代`list`实现通常使用**循环双向链表+哨兵节点**设计：**
...
```c++
list<T> lst;                     // 默认构造函数
list<T> lst(n);                  // 创建包含n个默认元素的list
list<T> lst(n, value);           // 创建包含n个value的list
list<T> lst(begin, end);         // 用迭代器范围初始化
list<T> lst(other_lst);          // 拷贝构造函数
list<T> lst(initializer_list);   // 初始化列表构造

lst.empty();                     // 判断是否为空
lst.size();                      // 返回元素个数
lst.max_size();                  // 返回最大可能元素数

lst.front();                     // 访问第一个元素
lst.back();                      // 访问最后一个元素
// 注意：list不支持随机访问 operator[] 和 at()

lst.push_front(value);           // 在开头插入元素
lst.emplace_front(args...);      // 在开头构造并插入元素
lst.pop_front();                 // 删除第一个元素

lst.push_back(value);            // 在末尾插入元素
lst.emplace_back(args...);       // 在末尾构造并插入元素
lst.pop_back();                  // 删除最后一个元素

lst.insert(pos, value);          // 在pos前插入元素
lst.insert(pos, n, value);       // 在pos前插入n个元素
lst.insert(pos, begin, end);     // 在pos前插入迭代器范围
lst.emplace(pos, args...);       // 在pos前构造并插入元素
lst.erase(pos);                  // 删除pos指向的元素
lst.erase(begin, end);           // 删除迭代器范围
lst.clear();                     // 删除所有元素

lst.splice(pos, other_lst);      // 将other_lst所有元素移到pos前
lst.splice(pos, other_lst, it);  // 将other_lst的单个元素移到pos前
lst.splice(pos, other_lst, begin, end); // 移动元素范围

lst.remove(value);               // 删除所有等于value的元素
lst.remove_if(predicate);        // 删除满足条件的元素

lst.sort();                      // 升序排序
lst.sort(compare);               // 按比较函数排序
lst.merge(other_lst);            // 合并两个已排序的list
lst.merge(other_lst, compare);   // 按比较函数合并


lst = other_lst;                 // 拷贝赋值
lst = {value1, value2, ...};     // 初始化列表赋值
lst.assign(n, value);            // 分配n个value
lst.assign(begin, end);          // 分配迭代器范围
lst.assign(initializer_list);    // 分配初始化列表
```

##### set和multiset

set是C++标准库中的关联容器，具有以下核心特性：

- **有序性**：元素自动按特定规则排序（默认升序）
...
- **平衡二叉搜索树**：底层通常使用红黑树实现
  ...
  set<Person, PersonCompare> s;   // 自定义比较函数
...
// result.second 表示是否插入成功（true表示元素不存在）

// 插入多个元素
s.insert({1, 2, 3, 4, 5});

// 使用emplace（C++11）
s.emplace(20);  // 原地构造，避免拷贝
```
...
// count - 返回元素个数（对于set只能是0或1）
if (s.count(10) > 0) {
    cout << "元素存在" << endl;
}

// lower_bound/upper_bound - 范围查找
auto lower = s.lower_bound(5);  // 第一个>=5的元素
auto upper = s.upper_bound(10); // 第一个>10的元素
```
...
// 通过迭代器范围删除
auto first = s.lower_bound(5);
auto last = s.upper_bound(15);
s.erase(first, last);  // 删除[5,15]范围内的元素
```
...
| **元素唯一性**     | ✓ 不允许重复         | ✗ 允许重复 |
| **count()返回值**  | 0或1                 | 0到n       |
| **equal_range()**  | 范围大小为0或1       | 可能大于1  |
| **insert()返回值** | pair<iterator, bool> | iterator   |

![image-20260210012311397](/notes-assets/C%2B%2B/assets/image-20260210012311397.png)
...
void ShowEmployee(multimap<int,Employee> &m) 
{
	for (auto it = m.begin(); it != m.end(); it++) {
		string DeptName = getDeptName((*it).first);
		cout << DeptName << (*it).second.getName() <<"工资:" << (*it).second.getSalary() << endl;
	}
}
void SetGroup(vector<Employee> &v,multimap<int,Employee> &m) {
	for (auto it = v.begin(); it != v.end(); it++) {
		int depId = rand() % 3 + 1; //0 , 1, 2
		m.insert({depId,(*it)});
	}
}
int main() {
	...
	for (int i = 0; i < 10; i++) {
		...
		double salary = rand() % 10001 + 10000;
		Employee em(name, salary);
		v.push_back(em);
	}
...
仿函数（Functor）是C++中行为类似函数的对象，它重载了函数调用运算符 `operator()`。

定义：**仿函数就是“像函数一样被调用的对象”**——本质是一个 **重载了 `operator()` 的类或结构体对象**
